use super::lines::{Lines, LinesSpectrogram};

#[derive(Debug, Deserialize, PartialEq, Serialize)]
#[serde(rename_all = "PascalCase")]
pub struct SpectrogramResult {
    #[serde(rename = "pid")]
    pub(crate) pid: String,
    #[serde(rename = "name")]
    pub(crate) name: String,
    pub(crate) time_span_start: f64,
    pub(crate) time_span_length: f64,
    pub(crate) range_first: usize,
    pub(crate) range_last: usize,
    pub(crate) trigger_line: usize,
    pub(crate) num_lines: usize,
    pub(crate) ref_timestamp: f64,
    #[serde(rename = "EarliestTS")]
    pub(crate) earliest_timestamp: f64,
    #[serde(rename = "LatestTS")]
    pub(crate) latest_timestamp: f64,
    #[serde(rename = "CurrentRS")]
    pub(crate) current_rs: usize,
    pub(crate) lines: Lines,
}

impl SpectrogramResult {
    pub fn data_points<'a>(&'a self) -> LinesSpectrogram<'a> {
        self.lines.data_points()
    }
}

#[cfg(test)]
mod tests {
    use serde_xml_rs;

    use super::*;
    use super::super::line::Line;

    #[test]
    fn deserialize_empty() {
        let input =
            "<SpectrogramResult pid=\"ogram\" name=\"Trace\">
                <TimeSpanStart>-0.011146285714285714</TimeSpanStart>
                <TimeSpanLength>0.022291428571428573</TimeSpanLength>
                <RangeFirst>0</RangeFirst>
                <RangeLast>307</RangeLast>
                <TriggerLine>1</TriggerLine>
                <NumLines>317</NumLines>
                <RefTimestamp>63647134869.552425794857142857</RefTimestamp>
                <EarliestTS>63647134063.50486790214285714</EarliestTS>
                <LatestTS>63647134869.54127950914285714</LatestTS>
                <CurrentRS>8091</CurrentRS>
                <Lines/>
            </SpectrogramResult>";

        let expected = SpectrogramResult {
            pid: "ogram".to_string(),
            name: "Trace".to_string(),
            time_span_start: -0.011146285714285714,
            time_span_length: 0.022291428571428573,
            range_first: 0,
            range_last: 307,
            trigger_line: 1,
            num_lines: 317,
            ref_timestamp: 63647134869.552425794857142857,
            earliest_timestamp: 63647134063.50486790214285714,
            latest_timestamp: 63647134869.54127950914285714,
            current_rs: 8091,
            lines: vec![].into(),
        };

        let spectrogram: SpectrogramResult =
            serde_xml_rs::from_str(input).unwrap();

        assert_eq!(spectrogram, expected);
    }

    #[test]
    fn deserialize() {
        let input =
            "<SpectrogramResult pid=\"ogram\" name=\"Trace\">
                <TimeSpanStart>-0.011146285714285714</TimeSpanStart>
                <TimeSpanLength>0.022291428571428573</TimeSpanLength>
                <RangeFirst>0</RangeFirst>
                <RangeLast>307</RangeLast>
                <TriggerLine>1</TriggerLine>
                <NumLines>317</NumLines>
                <RefTimestamp>63647134869.552425794857142857</RefTimestamp>
                <EarliestTS>63647134063.50486790214285714</EarliestTS>
                <LatestTS>63647134869.54127950914285714</LatestTS>
                <CurrentRS>8091</CurrentRS>
                <Lines>
                    <Line>
                        <TS>63647134063.50486790214285714</TS>
                        <RSID>1341</RSID>
                        <Units>dBm</Units>
                        <XStart>119436695</XStart>
                        <XRange>400000</XRange>
                        <TimeLen>2.4055942855714285</TimeLen>
                        <Valid>true</Valid>
                        <RBW>100.00255834564311</RBW>
                        <First>true</First>
                        <NumPoints>801</NumPoints>
                        <MultRecs>false</MultRecs>
                        <Data></Data>
                    </Line>
                    <Line>
                        <TS>63647134066.00499933014285714</TS>
                        <RSID>1363</RSID>
                        <Units>dBm</Units>
                        <XStart>119436695</XStart>
                        <XRange>400000</XRange>
                        <TimeLen>2.4102674285714287</TimeLen>
                        <Valid>true</Valid>
                        <RBW>100.00255834564311</RBW>
                        <First>true</First>
                        <NumPoints>801</NumPoints>
                        <MultRecs>false</MultRecs>
                        <Data></Data>
                    </Line>
                </Lines>
            </SpectrogramResult>";

        let expected = SpectrogramResult {
            pid: "ogram".to_string(),
            name: "Trace".to_string(),
            time_span_start: -0.011146285714285714,
            time_span_length: 0.022291428571428573,
            range_first: 0,
            range_last: 307,
            trigger_line: 1,
            num_lines: 317,
            ref_timestamp: 63647134869.552425794857142857,
            earliest_timestamp: 63647134063.50486790214285714,
            latest_timestamp: 63647134869.54127950914285714,
            current_rs: 8091,
            lines: vec![
                Line {
                    timestamp: 63647134063.50486790214285714,
                    rsid: 1341,
                    units: "dBm".to_string(),
                    x_start: 119436695,
                    x_range: 400000,
                    time_length: 2.4055942855714285,
                    valid: true,
                    resolution_bandwidth: 100.00255834564311,
                    first: true,
                    num_points: 801,
                    mult_recs: false,
                    data: "".to_string(),
                },
                Line {
                    timestamp: 63647134066.00499933014285714,
                    rsid: 1363,
                    units: "dBm".to_string(),
                    x_start: 119436695,
                    x_range: 400000,
                    time_length: 2.4102674285714287,
                    valid: true,
                    resolution_bandwidth: 100.00255834564311,
                    first: true,
                    num_points: 801,
                    mult_recs: false,
                    data: "".to_string(),
                },
            ].into(),
        };

        let spectrogram: SpectrogramResult =
            serde_xml_rs::from_str(input).unwrap();

        assert_eq!(spectrogram, expected);
    }

    #[test]
    fn data_points() {
        let spectrogram_result = SpectrogramResult {
            pid: "ogram".to_string(),
            name: "Trace".to_string(),
            time_span_start: -0.011146285714285714,
            time_span_length: 0.022291428571428573,
            range_first: 0,
            range_last: 307,
            trigger_line: 1,
            num_lines: 317,
            ref_timestamp: 63647134869.552425794857142857,
            earliest_timestamp: 63647134063.50486790214285714,
            latest_timestamp: 63647134869.54127950914285714,
            current_rs: 8091,
            lines: vec![
                Line {
                    timestamp: 0.0,
                    rsid: 0,
                    units: "dBm".to_string(),
                    x_start: 0,
                    x_range: 0,
                    time_length: 1.0,
                    valid: false,
                    resolution_bandwidth: 0.0,
                    first: false,
                    num_points: 3,
                    mult_recs: false,
                    data: "000000bf0000803e9a9919bf".to_string(),
                },
                Line {
                    timestamp: 1.0,
                    rsid: 0,
                    units: "dBm".to_string(),
                    x_start: 0,
                    x_range: 0,
                    time_length: 1.0,
                    valid: false,
                    resolution_bandwidth: 0.0,
                    first: false,
                    num_points: 3,
                    mult_recs: false,
                    data: "000000000000403fcdccccbd".to_string(),
                },
                Line {
                    timestamp: 2.0,
                    rsid: 0,
                    units: "dBm".to_string(),
                    x_start: 0,
                    x_range: 0,
                    time_length: 1.0,
                    valid: false,
                    resolution_bandwidth: 0.0,
                    first: false,
                    num_points: 3,
                    mult_recs: false,
                    data: "0000003f0000a03fcdcccc3e".to_string(),
                },
            ].into(),
        };

        let expected = vec![
            vec![-0.5, 0.25, -0.6],
            vec![0.0, 0.75, -0.1],
            vec![0.5, 1.25, 0.4],
        ];

        let result: Vec<Vec<f32>> = spectrogram_result
            .data_points()
            .map(|points| points.collect())
            .collect();

        assert_eq!(result, expected);
    }
}
